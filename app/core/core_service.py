...
class CoreService(object):
    ...
    _status_channel_name = '/sensors/ec'
    ...
    def __init__(self):
        self._ph = PHSensor()
        self._ph._set_i2c_address(54)

    def start(self):
        self._comm_client = mqtt.Client(
            client_id="service_atlasec",
            clean_session=True
        )
        ...
        while True:
            time.sleep(1)
            ec = self._ph._query('R')

            self.output('{"sender": "service_atlasec", "message": "%s"}' % ec)

    def _on_connect(self, client, userdata, flags, rc):
        self.output('{"sender": "service_atlasec", "message": "Connected to GrandCentral."}')
